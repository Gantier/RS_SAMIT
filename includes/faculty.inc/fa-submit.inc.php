...
if (isset($_POST['fa-submit']))
{
    ...
    for ($i = 0; $i < sizeof($batch); $i++)
    {
        ...
        $sqlAttendance = "SELECT *
                              FROM registration_system.attendance
                              WHERE attendanceStudent = '" . $studentAccount . "'
                                AND attendanceSection = " . $studentSection . "
                                AND attendanceDate = CURRENT_DATE;";
        //if attendance not already recorded for this day, insert the attendance
        if (mysqli_num_rows($conn->query($sqlAttendance)) === 0)
        {
            $sqlInsertAttendance = "INSERT INTO registration_system.attendance " .
                "(attendanceStudent, attendanceSection, attendanceDate, attendanceIsPresent)
                VALUES ('" . $studentAccount . "', " . $studentSection . ", CURRENT_DATE, " . $studentAttendance . ");";
            $conn->query($sqlInsertAttendance);
        }
        else//else, update the attendance
        {
            $sqlUpdateAttendance = "UPDATE registration_system.attendance
                                    SET attendanceIsPresent = " . $studentAttendance . "
                                    WHERE attendanceStudent = '" . $studentAccount . "'
                                      AND attendanceSection = " . $studentSection . "
                                      AND attendanceDate = CURRENT_DATE;";
            $conn->query($sqlUpdateAttendance);
        }
    }
