<?php
    ...
    if (isset($_POST['edit-pw-submit']))
    {
        ...
        if (empty($current) || empty($repeat) || empty($new))
        {
            ...
            header("Location: ../../admin_home.php?error=emptyFields");
            exit();
        }
        else
        {
            ...
            if ($passwordLock[0] < 5)
            {
                ...
                if ($currentPassword[0] === $current)
                {
                    ...
                    if ($new === $repeat)
                    {
                        if (preg_match("/^.*(?=.{8,})(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).*$/", $new) === 0)
                        {
                            ...
                            header("Location: ../../admin_home.php?error=weakPassword");
                            exit();
                        }
                        else
                        {
                            ...
                            if ($conn->query($sqlNewPassword) === true)
                            {
                                ...
                                header("Location: ../../admin_home.php?success=passwordChanged");
                                exit();
                            }
                            else
                            {
                                header("Location: ../../admin_home.php?error=sqlError");
                                exit();
                            }
                        }
                    }
                    else
                    {
                        ...
                        header("Location: ../../admin_home.php?error=typoInRepeat");
                        exit();
                    }
                }
                else
                {
                    ...
                    header("Location: ../../admin_home.php?error=incorrectPassword");
                    exit();
                }
            }
            else
            {
                ...
                header("Location: ../../admin_home.php?error=passwordLocked");
                exit();
            }
        }
    }
